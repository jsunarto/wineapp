import { Field, Section, Select, TextArea, TextInput } from "@/components/formControls";

export default function TastingSections({ icons, update, wine }) {
  return (
    <>
      <Section id="appearance" title="Appearance" icon={icons.star}>
        <Field label="Color"><TextInput value={wine.appearanceColor} onChange={(value) => update("appearanceColor", value)} placeholder="Dark red, nearly black" /></Field>
        <Field label="Clarity"><TextInput value={wine.appearanceClarity} onChange={(value) => update("appearanceClarity", value)} placeholder="Clear, hazy, opaque..." /></Field>
        <Field label="Intensity"><Select value={wine.appearanceIntensity} onChange={(value) => update("appearanceIntensity", value)} options={["", "Pale", "Medium", "Deep / intense"]} /></Field>
      </Section>
      ...
      <Section id="nose" title="Nose" icon={icons.search}>
        ...
        <Field label="Fruit notes"><TextInput value={wine.fruitNotes} onChange={(value) => update("fruitNotes", value)} placeholder="Dark plum, cherry, citrus..." /></Field>
        <Field label="Non-fruit notes"><TextInput value={wine.nonFruitNotes} onChange={(value) => update("nonFruitNotes", value)} placeholder="Vanilla, herbs, earth..." /></Field>
        <Field label="Oak notes"><TextInput value={wine.oakNotes} onChange={(value) => update("oakNotes", value)} placeholder="Oak, vanilla, toast..." /></Field>
        <Field label="Condition"><Select value={wine.flaw} onChange={(value) => update("flaw", value)} options={["Clean", "Unsure", "Possible flaw"]} /></Field>
      </Section>

      <Section id="palate" title="Palate" icon={icons.wine}>
        ...
        <Field label="Fruit/oak sweet impression"><Select value={wine.perceivedSweetness ? "Yes" : "No"} onChange={(value) => update("perceivedSweetness", value === "Yes")} options={["No", "Yes"]} /></Field>
        <Field label="Acidity"><Select value={wine.acidity} onChange={(value) => update("acidity", value)} options={["Low", "Medium", "High"]} /></Field>
        <Field label="Tannin"><TextInput value={wine.tannin} onChange={(value) => update("tannin", value)} placeholder="Medium-high; grippy" /></Field>
        <Field label="Body"><Select value={wine.body} onChange={(value) => update("body", value)} options={["Light", "Medium", "Full"]} /></Field>
        <Field label="Alcohol feel"><Select value={wine.alcohol} onChange={(value) => update("alcohol", value)} options={["Low", "Medium", "High"]} /></Field>
